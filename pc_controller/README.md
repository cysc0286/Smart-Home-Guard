...
- `serial_require_ack`：当前板端回传通道可能丢失短 ACK，推荐保持 `false`；明确收到 `ERR` 仍会报错，只有 ACK 超时会按“已发送但未确认”处理
...
- `M`：循环切换 HOME / AWAY / SLEEP / CONFIG；兼容模式下即使 ACK 不完整也会继续切换，并在状态栏提示“未确认”警告
- `Z` / `Backspace`：撤销最后一个草稿点
- `C`：取消草稿或待应用区域，不会清除板端正在生效的区域
- `Q` / `ESC`：退出

窗口采用“生效区域 / 草稿 / 待应用区域”三态：黄色填充表示当前生效区域，青色折线表示正在绘制的草稿，紫色轮廓表示已闭合但尚未发送的区域。重画期间板端继续检测旧区域；新区域在命令发送完成后替换旧区域并保存。板端明确返回 `ERR` 或发生串口写入错误时，旧区域保持生效，新区域保留为待应用状态，可以再次按 `S` 重试；仅 ACK 超时时会显示警告，并继续发送 `MODE` 和 `START`，避免检测停在配置阶段。`auto_exit_after_send=false` 时窗口会一直保留，可以反复重画。
...
| `N` | 重新请求一张快照 |
| `S` | 发送危险区到板端 |
| `M` | 循环切换 HOME / AWAY / SLEEP / CONFIG 工作模式 |
| `Q` / Esc | 退出 |

## 工作模式

模式通过 `M` 键循环切换，也可以在配置文件中设置 `arm_mode`。四种模式的策略如下：

- `HOME`：家庭/宠物安全提醒，对人员、猫、狗进入危险区报警，采用较短的间歇提示。
- `AWAY`：离家/无人值守防护，仅对人员进入危险区报警，采用标准声光节奏。
- `SLEEP`：夜间安防，对人员进入危险区立即报警，采用最高强度的长响短停节奏，目的是唤醒住户并警告进入危险区的人；宠物活动不降低这条人体危险报警策略的优先级。
- `CONFIG`：配置/维护模式，继续显示检测框和黄色危险区，但不产生红框、GPIO 或蜂鸣器告警。

切换模式时板端会立即结束旧模式的告警、关闭 GPIO、清空检测防抖，并重新经过 800 ms 确认后才允许新模式告警，避免把旧状态带入新策略。
...
- 板端 OSD 使用透明底黄色 RLE 位图显示真实多边形轮廓；若位图提交失败才降级为外接矩形。显示与告警判断使用同一组多边形顶点。
