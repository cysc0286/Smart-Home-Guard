...
void IMAGEPROCESSOR::Initialize(std::array<int, 2>* in_img_shape) 
{
    ...
    format_online = SSNE_YUV422_16;              // 图像格式：YUV422 16位（YuNet三通道模型）
    
    // pipe0设置：用于获取裁剪后的图像（原图1920×1080 -> 裁剪为1440×1080）
    // 左右各裁剪240像素：x_start=240, x_end=240+1440=1680, y_start=0, y_end=1080
    OnlineSetCrop(kPipeline0, 240, 1680, 0, 1080);  // 裁剪区域：x=240, w=1440, y=0, h=1080
