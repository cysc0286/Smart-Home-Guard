...
- OSD 展示：检测框、多边形危险区和状态卡分图层显示。多边形危险区会在配置变化时生成透明底黄色轮廓 SSBMP，并通过独立 RLE 图层原子刷新；告警判断仍按同一组真实多边形点完成。状态卡只在模式或事件状态变化时切换预生成位图，显示当前模式、未配置区域、告警或系统降级，不进入逐帧绘制热路径。若多边形位图编码或提交失败，OSD 会自动退回外接矩形并输出 `[ZONE][WARN]`，避免危险区完全不可见。
- 声光报警：目标进入危险区时触发 GPIO LED/蜂鸣器。
- 四种工作模式：HOME 对人员、猫、狗进入危险区进行家庭/宠物安全提醒；AWAY 仅对人员进入危险区进行离家防护；SLEEP 仅对人员进行长响短停的夜间提醒；CONFIG 保持检测框和危险区显示，但关闭红框、GPIO 和蜂鸣器告警，便于现场配置。
